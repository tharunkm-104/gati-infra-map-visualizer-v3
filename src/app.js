...
function pointsForLevel(level) {
  if (datasetMode === "allIndia") {
    ...
    return allIndiaPoints.filter(
      (p) => isPointActive(p) && (!trueCoordsOnly || p.coordinateStatus !== "pin_centroid")
    );
  }
...
function mappedPointCounts() {
  ...
  const source = datasetMode === "allIndia"
    ? allIndiaPoints.filter((p) => !trueCoordsOnly || p.coordinateStatus !== "pin_centroid")
    : coordinateFilteredInfrastructure();
